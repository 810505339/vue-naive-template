...
<template>
  ...
  <div fixed z-10 w="400px" right="0" top="80px" rounded-tl-3xl h="100%" bg=" white dark:#18181c" pt="20px">
    <div>
      ...
      <div my8px>
        <div v-for="(play, i) in playList" :key="play.id" flex items-center w="100%" justify-evenly py-8px px="24px"
          hover="bg-gray-50 dark:bg-gray-2/3" @click="playMusic(play)">
          <span mr-15px text="#787887">0{{ i + 1 }}</span>
          <img :src="play.al.picUrl" w="50px" h="50px" mr-8px rounded>
          <div basis="1/2">
    ...
    <div px="24px">
      ...
      <div text-center>
        <img rounded-16px w="200px" h="120px" inline-block blur="0.5" :src="playListStore.selectItem?.al?.picUrl"
          v-if="playListStore.selectItem?.al?.picUrl" />
        ...
        <div flex items-center w="100%">
          <div basis="1/4" flex items-center>
            ...
            <n-popover trigger="hover" :show-arrow="false" style=" height: 160px">
              <template #trigger>
                <i icon-btn :class="[playModelIcon]" text="#b4b4f8 20px"></i>
              </template>
              <n-slider h="100%" style="height: 160px;" :tooltip="false" vertical />
            </n-popover>

          </div>


          <div text="#4343ef" flex items-center min-w="150px" justify-between basis="1/2">
